...
def calibracao(area_drenagem, index, serie_precipitacao, serie_evaporacao_potencial, serie_vazao_montante, serie_vazao_observada, limites):
    ...
    def roda(p):
        ...
        m = muskingum(
            serie_vazao_montante=serie_vazao_montante,
            tempo_viagem_pico=p9
            )
        
        return list(map(sum, zip(s,m)))
    ...
    res_log_nse = minimize(fun=obj_log_nse, x0=res_nse.x, bounds=limites)
